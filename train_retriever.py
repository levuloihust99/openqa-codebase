...
from dpr.data import dataloader
...
3. Caching dataset
4. Shuffle: You should shuffle before batch to guarantee that each data sample can be batched with different data samples in different epochs
5. Repeating data: repeat to produce indefininte data stream
6. Batching dataset
7. Merging contexts: Each data sample contains a positive context and a hard negative context. When batching, we want each batch is in the following format:
    {
        'question_ids': ...,
        'question_masks': ...,
        'context_ids': ...,
   ...
   Here, positive and hard negative contexts in a batch needs to be merged together to produce the common `context_ids`
8. Prefetching dataset (to speed up training) 
"""
dataset = dataloader.load_retriever_int_data(
    data_path=args.data_path,
    shuffle=args.shuffle,
    shuffle_seed=args.seed
)
dataset = dataloader.pad(dataset, max_context_length=args.max_context_length, max_query_length=args.max_query_length)
dataset = dataset.cache()
dataset = dataset.shuffle(buffer_size=60000)
dataset = dataset.repeat()
dataset = dataset.batch(args.batch_size)
dataset = dataloader.merge_contexts(dataset)
